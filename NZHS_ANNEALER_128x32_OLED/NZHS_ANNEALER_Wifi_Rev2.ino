...
// MQTT Configuration
const char* ssid = "WhiteHouseIOT";
const char* password = "FuckOffIOT!!";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "machine/control") {
    if (message == "START") {
      ...
      int newTime = message.substring(5).toInt();
      AnnealTime_ms = newTime;
    }
  }
}

float getTemperature() {
  // Your code to read the temperature
  return 25.0; // Example temperature value
}

void publishData() {
  String temperature = String(getTemperature());
  String status = "Running"; // Replace with actual status
  String cases = String(CasesAnnealed);

  Serial.print("Publishing temperature: ");
  Serial.println(temperature);
  client.publish("machine/temperature", temperature.c_str());

  Serial.print("Publishing status: ");
  Serial.println(status);
  client.publish("machine/status", status.c_str());

  Serial.print("Publishing cases: ");
  Serial.println(cases);
  client.publish("machine/cases", cases.c_str());
}
